...
if __name__ == "__main__":
	...
	for r in releases:
		...
		crashLogs  	= cur.execute("SELECT count(*) FROM `%s` WHERE ExitCode<>'None' and ExitCode>0;" % r).fetchall()[0][0]
		uniqueCrashLogs = cur.execute("SELECT count(*) FROM `%s` WHERE ExitCode<>'None' and ExitCode>0 group by CrashStack;" % r).fetchall()
		...
		maxUsageCrashed	= cur.execute("SELECT max(UsageTime) FROM `%s` WHERE ExitCode<>'None' and ExitCode>0;" % r).fetchall()[0][0]
		minUsage 	= cur.execute("SELECT min(UsageTime) FROM `%s` WHERE 1=1;" % r).fetchall()[0][0]
		minUsageCrashed	= cur.execute("SELECT min(UsageTime) FROM `%s` WHERE ExitCode<>'None' and ExitCode>0;" % r).fetchall()[0][0]
		avgUsage 	= cur.execute("SELECT avg(UsageTime) FROM `%s` WHERE 1=1;" % r).fetchall()[0][0]
		avgUsageCrashed	= cur.execute("SELECT avg(UsageTime) FROM `%s` WHERE ExitCode<>'None' and ExitCode>0;" % r).fetchall()[0][0]
		...
		print_formatted('CPU Time',        "%s hours of cpu usage" % "{:0.3f}".format(cpuUsage / 3600))
		print_formatted('Elapsed Time',    "%s hours tool was up" % "{:0.3f}".format(realUsage / 3600))
		print_formatted("Usage Time",      "%s hours tool activly used" % "{:0.3f}".format(totalUsage / 3600))
		print_formatted("MTBC", 	   "%s hours (Usage Time / Crashed Sessions)" % "{:0.3f}".format(mtbc / 3600))
		
		print ("");
		print_formatted("Longest Session", "%s hours across all sessions" % "{:0.3f}".format(maxUsage/3600))
		print_formatted("", 		   "%s hours across crashed sessions" % "{:0.3f}".format(maxUsageCrashed/3600))
		print_formatted("Shortest Session","%s hours across all sessions" % "{:0.3f}".format(minUsage/3600))
		print_formatted("",                "%s hours across crashed sessions" % "{:0.3f}".format(minUsageCrashed/3600))
		print_formatted("Average Session", "%s hours across all sessions" % "{:0.3f}".format(avgUsage/3600))
		print_formatted("",                "%s hours across crashed sessions" % "{:0.3f}".format(avgUsageCrashed/3600))
